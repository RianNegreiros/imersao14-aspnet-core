...
export function NewRoutePage() {
  ...
  async function searchPlaces(event: FormEvent) {
    ...
    const [sourceResponse, destinationResponse] = await Promise.all([
      fetch(`http://localhost:5000/api/places?text=${source}`),
      fetch(`http://localhost:5000/api/places?text=${destination}`),
    ]);
    ...
    const directionsResponse = await fetch(
      `http://localhost:5000/api/directions?originId=${placeSourceId}&destinationId=${placeDestinationId}`
    );
